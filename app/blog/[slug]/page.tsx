        ...
        {post.content.map((section, index: number) => (
          <div key={index} className="mb-12">
            ...
            <div className="text-gray-700 leading-relaxed text-lg whitespace-pre-line">
              {section.internalLinks?.reduce((text: string, link: { context: string; url: string; text: string }, linkIndex: number) => {
      ...
      {post.relatedPosts && post.relatedPosts.length > 0 && (
        <div className="mt-16 pt-8 border-t border-gray-200">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {post.relatedPosts.map((relatedPost: { slug: string; title: string; context: string }) => (
              <Link 
